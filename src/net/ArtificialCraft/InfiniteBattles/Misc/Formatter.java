...
public class Formatter{
	...
	public static Contestant parseContestant(String c){
		...
		onstreak = Boolean.parseBoolean(info[4]);
		for(String s : counts){
			String[] split = s.split(",");
			count.put(BattleType.valueOf(split[0]), Integer.parseInt(split[1]));
		}
		return new Contestant(name, wins, losses, streak, onstreak, count);
	}
	...
	public static Arena parseArena(String s){
		...
		if(!info[2].equalsIgnoreCase("null"))
		...
		pastepoint = parseLoc(locs[6]);
		Arena a = new Arena(name, bluespawn, redspawn, greenspawn, purplespawn, spectatorspawn, pitstop, pastepoint, max);
		if(unique != null)
			a.setUnique(unique);
		return a;
	}
	...
	public static Location parseLoc(String l){
		...
		try{
			...
			return new Location(Bukkit.getServer().getWorld("iBattle"), x, y, z);
